...
## Steamcmd debugging
if [[ ${DEBUG:-0} -eq 1 || ${DEBUG:-0} -eq 3 ]]; then
    STEAMCMD_SPEW="+set_spew_level 4 4"
fi
## CS2 server debugging
if [[ ${DEBUG:-0} -eq 2 || ${DEBUG:-0} -eq 3 ]]; then
    CS2_LOG="on"
    CS2_LOG_MONEY=1
    CS2_LOG_DETAIL=3
    CS2_LOG_ITEMS=1
...
# Download Updates
if [[ "${STEAMAPPVALIDATE:-0}" -eq 1 ]]; then
    VALIDATE="validate"
else
    VALIDATE=""
fi
...
while [[ $steamcmd_rc -ne 0 && $attempt -lt $MAX_ATTEMPTS ]]; do
    ...
    if [[ $attempt -gt 1 ]]; then
        ...
        # Stale appmanifest data can lead for HTTP 401 errors when requesting old files from SteamPipe CDN
        echo "Removing steamapps (appmanifest data)..."
        rm -rf "${STEAMAPPDIR}/steamapps"
    fi
    bash "${STEAMCMDDIR}/steamcmd.sh" ${STEAMCMD_SPEW:-}         +force_install_dir "${STEAMAPPDIR}"         +@bClientTryRequestManifestWithoutCode 1         +login anonymous         +app_update "${STEAMAPPID}" ${VALIDATE}         +quit || true
    steamcmd_rc=$?
done
...
ln -sfT "${STEAMCMDDIR}/linux64/steamclient.so" "${HOME:-/root}/.steam/sdk64/steamclient.so"
...
if [[ -n "${CS2_CFG_URL:-}" ]]; then
    echo "Downloading config pack from ${CS2_CFG_URL}"
    TEMP_DIR=$(mktemp -d)
    TEMP_FILE="${TEMP_DIR}/$(basename "${CS2_CFG_URL}")"
    wget -qO "${TEMP_FILE}" "${CS2_CFG_URL}"
    case "${TEMP_FILE}" in
        *.zip)            unzip -o -q "${TEMP_FILE}" -d "${STEAMAPPDIR}" ;;
        *.tar.gz|*.tgz)   tar xzf "${TEMP_FILE}" -C "${STEAMAPPDIR}"     ;;
        *.tar)            tar xf "${TEMP_FILE}" -C "${STEAMAPPDIR}"      ;;
        *) echo "Unsupported file type"; rm -rf "${TEMP_DIR}"; exit 1    ;;
    esac
    rm -rf "${TEMP_DIR}"
fi

# Rewrite Config Files
sed -i -e "s/{{SERVER_HOSTNAME}}/${CS2_SERVERNAME}/g"        -e "s/{{SERVER_CHEATS}}/${CS2_CHEATS}/g"        -e "s/{{SERVER_HIBERNATE}}/${CS2_SERVER_HIBERNATE}/g"        -e "s/{{SERVER_PW}}/${CS2_PW}/g"        -e "s/{{SERVER_RCON_PW}}/${CS2_RCONPW}/g"        -e "s/{{TV_ENABLE}}/${TV_ENABLE}/g"        -e "s/{{TV_PORT}}/${TV_PORT}/g"        -e "s/{{TV_AUTORECORD}}/${TV_AUTORECORD}/g"        -e "s/{{TV_PW}}/${TV_PW}/g"        -e "s/{{TV_RELAY_PW}}/${TV_RELAY_PW}/g"        -e "s/{{TV_MAXRATE}}/${TV_MAXRATE}/g"        -e "s/{{TV_DELAY}}/${TV_DELAY}/g"        -e "s/{{SERVER_LOG}}/${CS2_LOG}/g"        -e "s/{{SERVER_LOG_MONEY}}/${CS2_LOG_MONEY}/g"        -e "s/{{SERVER_LOG_DETAIL}}/${CS2_LOG_DETAIL}/g"        -e "s/{{SERVER_LOG_ITEMS}}/${CS2_LOG_ITEMS}/g"        "${STEAMAPPDIR}/game/csgo/cfg/server.cfg"
...
if [[ -n "${CS2_HOST_WORKSHOP_COLLECTION:-}" || -n "${CS2_HOST_WORKSHOP_MAP:-}" ]]; then
...
eval "./cs2.sh" -dedicated         "${CS2_IP_ARGS}" -port "${CS2_PORT}"         -console         -usercon         -maxplayers "${CS2_MAXPLAYERS}"         "${CS2_GAME_MODE_ARGS}"         "${CS2_MAPGROUP_ARGS}"         +map "${CS2_STARTMAP}"         "${CS2_HOST_WORKSHOP_COLLECTION_ARGS:-}"         "${CS2_HOST_WORKSHOP_MAP_ARGS:-}"         "${CS2_MP_MATCH_END_CHANGELEVEL:-}"         +rcon_password "${CS2_RCONPW}"         "${SV_SETSTEAMACCOUNT_ARGS:-}"         "${CS2_PW_ARGS:-}"         +sv_lan "${CS2_LAN}"         +tv_port "${TV_PORT}"         "${CS2_ADDITIONAL_ARGS:-}"
